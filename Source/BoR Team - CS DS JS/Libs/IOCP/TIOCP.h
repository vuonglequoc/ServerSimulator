...
public:
	...
	bool	Initialize(UINT NumberOfThreads);
	// ----
	bool	DataSend(int aIndex, UCHAR* lpMsg, DWORD dwSize);
	void	DataSendAll(UCHAR * lpMsg, DWORD dwSize){for(int i = 0 ; i != MAX_OBJECT ; i++){DataSend(i,lpMsg,dwSize);}};
...
public:
	...
	void inline		setIsOneConPerIP(bool status){m_IsOneIpPerCon = status;};
	...
	bool inline		isConnected(int aIndex){if(m_Server[aIndex].m_State > SS_CLOSED)return true;else return false;};
};
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
