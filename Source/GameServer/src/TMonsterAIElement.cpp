...
//	GS-N	2.00.05	JPN	0x0055D120	-	Completed
//////////////////////////////////////////////////////////////////////
...
BOOL TMonsterAIElement::LoadData(const char* lpszFileName)
{
	TMonsterAIElement::s_bDataLoad = FALSE;

	if ( lpszFileName == NULL || strcmp(lpszFileName, "") == 0 )
	{
		CLog.MsgBox("[Monster AI Element] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( !SMDFile )
		{
			CLog.MsgBox("[Monster AI Element] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster AI Element] - ElementNumber(%d) Error (%s) File. ", iElementNumber, lpszFileName);
						continue;
					}

					if ( iElementClass < 0 || iElementClass >= MAX_AI_ELEMENT_CLASS )
					{
						CLog.MsgBox("[Monster AI Element] - ElementClass(%d) Error (%s) File. ", iElementClass, lpszFileName);
						continue;
					}
		...
		fclose(SMDFile);
		CLog.LogAddC(TColor.Red(),"[Monster AI Element] - %s file is Loaded", lpszFileName);
		TMonsterAIElement::s_bDataLoad = TRUE;
	}
	catch (DWORD)
	{
		CLog.MsgBox("[Monster AI Element] - Loading Exception Error (%s) File. ", lpszFileName);
	}

	TMonsterAIElement::s_MonsterAIMovePath[MAP_INDEX_CRYWOLF_FIRSTZONE].LoadData("..\\Data\\MovePath_35.dat");
...
TMonsterAIElement * TMonsterAIElement::FindAIElement(int iElementNumber)
{
	if ( iElementNumber < 0 || iElementNumber >= MAX_AI_ELEMENT )
	{
		CLog.LogAdd("[Monster AI Element] FindAIElement() Error - (ElementNumber=%d) ", iElementNumber);
		return NULL;
	}
...
BOOL TMonsterAIElement::ApplyElementAttackArea(int iIndex, int iTargetIndex, TMonsterAIState * pAIState)
{
	...
	for (int i= OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		...
		if (iTargetDistance < 10)
		{
			...
			C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x0C, sizeof(pMsg));
			...
			gSendProto.DataSend(lpTargetObj->m_Index, (LPBYTE)&pMsg, sizeof(pMsg));
		}
...
BOOL TMonsterAIElement::ApplyElementNightmareWarp(int iIndex, int iTargetIndex, TMonsterAIState * pAIState)
{
	...
	if ( lpObj->Type == OBJ_USER )
		gSendProto.DataSend(iIndex,(unsigned char *)&pAttack,pAttack.h.size);

	gSendProto.VPMsgSend(lpObj,(unsigned char*)&pAttack,pAttack.h.size);
...
BOOL TMonsterAIElement::ApplyElementAIChange(int iIndex, int iTargetIndex, TMonsterAIState * pAIState)
{
	...
	CLog.LogAddC(TColor.Red(), "[AI Change] Group %d AI Order %d",
