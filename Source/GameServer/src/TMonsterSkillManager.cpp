...
//	GS-N	2.00.05	JPN	0x00558440	-	Completed
//////////////////////////////////////////////////////////////////////
...
BOOL TMonsterSkillManager::LoadData(const char* lpszFileName)
{
	TMonsterSkillManager::s_bDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, ""))
	{
		CLog.MsgBox("[Monster Skill Manager] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Monster Skill Manager] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster Manager] - MonsterIndex(%d) Error (%s) File. ", 
					...
					if ( bVerified ==FALSE )
					{
						CLog.LogAddC(TColor.Red(), "[Monster Manager] - Invalid SkillInfo : MIndex(%s/%d)",
		...
		CLog.LogAddC(TColor.Red(), "[Monster Skill Manager] - %s file is Loaded", lpszFileName);

		TMonsterSkillManager::s_bDataLoad = TRUE;
	}
	catch(DWORD)
	{
		CLog.MsgBox("[Monster Skill Manager] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
BOOL TMonsterSkillManager::AddMonsterSkillDelayInfo(int iIndex, int iTargetIndex, int iDelayTime, TMonsterSkillUnit * lpMonsterSkillUnit)
{
	...
	CLog.LogAddC(TColor.Red(), "[Monster Skill Manager] - (MonsterSkillDelayInfo) Slot Full!! ");
	return FALSE;
}
...
void TMonsterSkillManager::UseMonsterSkill(int iIndex, int iTargetIndex, int iMonsterSkillUnitType)
{
	...
	{
		CLog.LogAddC(TColor.Red(), "[Monster Skill Manager] SkillUnit is NULL ");
		return;
	}
