...
BOOL CKanturuMonsterMng::LoadData(const char * lpszFileName)
{
	this->m_bFileDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName , "") )
	{
		CLog.MsgBox("[ KANTURU ][ MonsterSetBase ] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			...
			CLog.MsgBox("[ KANTURU ][ MonsterSetBase ] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ MonsterSetBase ] - Exceed Max Info Count (%d)",
		...
		fclose(SMDFile);
		CLog.LogAddC(2, "[ KANTURU ][ MonsterSetBase ] - %s file is Loaded",
			lpszFileName);
	...
	{
		CLog.MsgBox("[ KANTURU ][ MonsterSetBase ] Loading Exception Error (%s) File. ", lpszFileName);
	}
...
int CKanturuMonsterMng::SetKanturuMonster(int iGroupNumber)
{
	...
	for ( int iCount=0;iCount<MAX_KANTURU_MONSTER;iCount++)
	{
			 ...
			 this->m_SetBaseInfo[iCount].wType != 0 )
		{
			...
			if ( iResult < 0 )
			{
				CLog.LogAdd("[ KANTURU ][ SetBossMapMonster ] Fail - Type:%d Map[%d]-[%d][%d]",
			...
			else
			{
				...
				if ( this->m_SetBaseInfo[iCount].wType == 364 )	/// Maya Hand
				{
					...
					CLog.LogAdd("[ KANTURU ][ SetBossMapMonster ] %s(Index:%d / ObjIndex:%d) Map:%d-[%d][%d]",
				...
				if ( gObj[iResult].Type == OBJ_MONSTER )
				{
					...
					CLog.LogAdd("[ KANTURU ][ SetBossMapMonster ] Count:%d %s(Index:%d / ObjIndex:%d) Map:%d-[%d][%d]",
