...
//	GS-N	2.00.05	JPN	0x0050BB90	-	Completed
//	GS-CS	2.00.05	JPN	0x0050BB90	-	Completed
...
int CSetItemOption::IsSetItem(int itemnum)	
{
	...
	{
		CLog.LogAdd("IsSetItem Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GetOptionTableIndex(int itemnum, int setoption)	
{
	if (itemnum<0 || itemnum > MAX_ITEMS-1)
	{
		CLog.LogAdd("GetOptionTableIndex Check error: (itemnum:%d)", itemnum);
		return -1;
	}

	if (setoption != 1 && setoption != 2)
	{
		CLog.LogAdd("GetOptionTableIndex setoption Check error: (itemnum:%d)", itemnum);
		return -1;
	}
...
char * CSetItemOption::GetSetOptionName(int optionindex)
{
	...
	{
		CLog.LogAdd("GetSetOptionName Check error: (itemnum:%d)", optionindex);
		return "";
	}
...
LPSTR CSetItemOption::GetSetOptionName(int itemnum, int settype)
{
	...
	if (itemnum<0 || itemnum > MAX_ITEMS-1)
	{
		CLog.LogAdd("GetSetOptionName Check error: (itemnum:%d)", itemnum);
		return "";
	}
...
int CSetItemOption::GetSetOption(int optionindex, int optablenum, int& op1, int& op2, int& opvalue1,int& opvalue2, int Class, int ChangeUp, int ChangeUp2)
{
	opvalue1=0;
	opvalue2=0;
	...
	if (optionindex <0 || optionindex > MAX_SETITEM_OPTION-1)
	{
		CLog.LogAdd("GetSetOption Check error: (itemnum:%d)", optionindex);
		return 0;
	}
	...
	isRequireClass=this->IsRequireClass(optionindex, Class, ChangeUp, ChangeUp2);
...
int CSetItemOption::GetMaxSetOptionCount(int optionindex)
{
	if (optionindex <0 || optionindex > MAX_SETITEM_OPTION-1)
	{
		CLog.LogAdd("GetMaxOptionCount Check error: (itemnum:%d)", optionindex);
		return -1;
	}
...
int CSetItemOption::GetGetFullSetOption(int optionindex, int& op1, int& op2, int& op3, int& op4, int& op5, int& opvalue1, int& opvalue2, int& opvalue3, int& opvalue4, int& opvalue5, int Class, int ChangeUp, int ChangeUp2)
{
	opvalue1 = 0;
	opvalue2 = 0;
	opvalue3 = 0;
	...
	BOOL isRequireClass=this->IsRequireClass(optionindex, Class, ChangeUp, ChangeUp2);
...
BOOL CSetItemOption::IsRequireClass(int optionindex, int Class, int ChangeUP, int ChangeUP2)
{
	int requireclass;
	...
	if ( Class <0 || Class > MAX_CLASS_TYPE-1)
	{
		return 0;
	}
...
int CSetItemOption::CheckMixContition(int itemnum, int mixitemlevel)
{
	...
	if ( itemnum <0 || itemnum > MAX_ITEMS-1)
	{
		CLog.LogAdd("MakeSetOption Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GenSetOption(int itemnum)
{
	if ( itemnum < 0 || itemnum > MAX_ITEMS-1)
	{
		CLog.LogAdd("GenSetType Check error: (itemnum:%d)", itemnum);
		return FALSE;
	}
