...
//	GS-N	2.00.05	JPN	0x0055FB90	-	Completed
//////////////////////////////////////////////////////////////////////
...
BOOL TMonsterAIGroup::LoadData(const char* lpszFileName)
{
	TMonsterAIGroup::s_bDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, ""))
	{
		CLog.MsgBox("[Monster AI Group] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Monster AI Group] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster AI Group] Exceed Max GroupNumber %d",
							iGroupNumber);
						continue;
					}
					...
					{
						CLog.MsgBox("[Monster AI Group] Exceed Max GroupMember %d, %d ",
							iGroupNumber, iGuid);
						continue;
					}
		...
		CLog.LogAddC(TColor.Red(), "[Monster AI Group ] - %s file is Loaded", lpszFileName);

		TMonsterAIGroup::s_bDataLoad = TRUE;
	}
	catch ( DWORD )
	{
		CLog.MsgBox("[Monster AI Group] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
void TMonsterAIGroup::ChangeAIOrder(int iGroupNumber, int iAIOrder)
{
	...
	{
		CLog.LogAdd("[Monster AI Group] ChangeAIOrder() Error - (GroupNumber=%d)", iGroupNumber);
		return;
	}
...
BOOL TMonsterAIGroup::DelGroupInstance(int iGroupNumber)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		CLog.LogAdd("[Monster AI Group] DelGroup() Error - (GroupNumber=%d)", iGroupNumber);
		return FALSE;
	}
...
int TMonsterAIGroup::FindGroupLeader(int iGroupNumber)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		CLog.LogAdd("[Monster AI Group] FindGroupLeader() Error - (GroupNumber=%d)", iGroupNumber);
...
int TMonsterAIGroup::FindGroupMemberObjectIndex(int iGroupNumber, int iGuid)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		CLog.LogAdd("[Monster AI Group] FindGroupMemberObjectIndex() Error - (GroupNumber=%d Guid=%d)", iGroupNumber, iGuid);
		return 0;	// #error Change this to -1
	}
...
TMonsterAIGroupMember * TMonsterAIGroup::FindGroupMember(int iGroupNumber, int iGuid)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		CLog.LogAdd("[Monster AI Group] FindGroupMember() Error - (GroupNumber=%d)(Guid=%d) ", iGroupNumber, iGuid);
		return NULL;
	}
...
void TMonsterAIGroup::Init(int iGroupNumber)
{
	...
	for ( int j=0;j<MAX_MONSTER_AI_GROUP_MEMBER;j++)
	{
		...
		if ( iResult >= 0 )
		{
			...
			CLog.LogAdd("[ KANTURU ][ SetAIMonster ] %s(Index:%d ObjIndex:%d) Map:%d-[%d][%d]",
