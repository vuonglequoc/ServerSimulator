...
/*	GS-N	2.00.05	JPN	0x00554F40	-	Completed
...
BOOL TMonsterSkillElement::LoadData(const char* lpszFileName)
{	
	TMonsterSkillElement::s_bDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, ""))
	{
		CLog.MsgBox("[Monster Element] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Monster Element] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster Element] - ElementNumber(%d) Error (%s) File. ",
		...
		CLog.LogAddC(TColor.Red(), "[Monster Element] - %s file is Loaded", lpszFileName);

		TMonsterSkillElement::s_bDataLoad = TRUE;
	}
	catch ( DWORD)
	{
		CLog.MsgBox("[Monster Element] - Loading Exception Error (%s) File. ", lpszFileName);	
	}
...
TMonsterSkillElement * TMonsterSkillElement::FindSkillElement(int iElementNumber)
{
	if ( iElementNumber < 0 || iElementNumber >= MAX_MONSTER_SKILL_ELEMENT )
	{
		CLog.LogAdd("[Monster Element] FindSkillElement() Error - (ElementNumber=%d) ",
			iElementNumber);
