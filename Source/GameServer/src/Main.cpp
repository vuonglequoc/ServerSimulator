...
*/
	if( Connected[0] == FALSE )
	{
		...
		{	
			CLog.MsgBox("GameServer", "GameServer Couldn't Connect to JoinServer!", MB_OK);
			ExitProcess(0);
		}
	}
	...
	if( (Connected[2] != FALSE) && (Connected[3] == FALSE) )
	{
		...
		{
			CLog.MsgBox("GameServer", "GameServer Couldn't Connect to DevilSquare Event!", MB_OK);
			return false;
		}
		Connected[3] = true;
	}
	if( (Connected[4] != FALSE) && (Connected[5] == FALSE) )
	{
		...
		{
			CLog.MsgBox("GameServer", "GameServer Couldn't Connect to Event ChipServer!", MB_OK);
			return false;
		}
		Connected[5] = true;
	}
