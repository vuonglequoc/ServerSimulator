...
BOOL CKanturuBattleUserMng::AddUserData(int iIndex)
{
	...
	{
		CLog.LogAdd("[ KANTURU ][ BattleUser ] Add User Fail - Disconnect User [%s][%s]",
	...
	{
		CLog.LogAdd("[ KANTURU ][ BattleUser ] Add User Fail - Over Max User [%s][%s]",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
...
BOOL CKanturuBattleUserMng::DeleteUserData(int iIndex)
{
	...
	{
		CLog.LogAdd("[ KANTURU ][ BattleUser ] Delete User Fail - Unvalid Index:%d",
			iIndex);
...
#endif
				CLog.LogAdd("[ KANTURU ][ BattleUser ] Delete User - [%s][%s] Current Battle User:%d",
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		if ( this->m_BattleUser[iCount].IsUseData() == TRUE )
		{
			...
			if ( !gObjIsConnected(iIndex))
			{
				...
				CLog.LogAdd("[ KANTURU ][ BattleUser ] Delete User - Disconnect [%s][%s]",
					gObj[iIndex].AccountID, gObj[iIndex].Name);
			}
				 ...
				 gObj[iIndex].Live == 1 )
			{
				...
				CLog.LogAdd("[ KANTURU ][ BattleUser ] Delete User - Map Move [%s][%s]",
					gObj[iIndex].AccountID, gObj[iIndex].Name);
			}
		}
	}
...
BOOL CKanturuBattleUserMng::MoveAllUser(int iGateNumber)
{
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		...
		if ( this->m_BattleUser[iCount].IsUseData()  )
		{
			...
			{
				CLog.LogAdd("[ KANTURU ][ BattleUser ] [%s][%s] MoveGate(%d)",
			...
			else
			{
				...
				CLog.LogAdd( "[ KANTURU ][ BattleUser ] [%s][%s] MoveGate Fail (%d)",
					gObj[iIndex].AccountID, gObj[iIndex].Name, iGateNumber);
...
void CKanturuBattleUserMng::SetMaxUser(int iMaxUser)
{
	...
	if ( iMaxUser > MAX_KANTURU_BATTLE_USER )
	{
		...
		CLog.LogAdd("[ KANTURU ][ BattleUser ] Set Max User:%d",
			iMaxUser);
	}
}
...
void CKanturuBattleUserMng::LogBattleWinnerUserInfo(BYTE btFlag, int iElapsedTime)
{
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		if ( this->m_BattleUser[iCount].IsUseData() == TRUE )
		{
			...
			CLog.LogAdd("[ KANTURU ][ BATTLE WINNER ] [%d/ElapsedTime:%0.3f] [%s][%s] MapNumber[%d]-(%d/%d)",
