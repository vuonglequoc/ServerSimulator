...
#include "../Header Files/LifeStone.h"
#include "../Header Files/Mercenary.h"
#include "../Header Files/DBSockMng.h"
#include "../Header Files/LogToFile.h"
...
int CCastleSiege::LoadPreFixData(char * lpszFileName)
{
	...
	else
	{
		...
		{
			m_bFixCastleCycleStartDate = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::LoadPreFixData() - iEVENT_END_DATE_NUM (%04d-%02d-%02d) <= iTODAY_DATE_NUM (%04d-%02d-%02d)",
	...
	else
	{
		...
		{
			m_bFixCastleStateStartDate = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::LoadPreFixData() - iSIEGE_DATE_NUM64 (STATE:%d, %04d-%02d-%02d %02d:%02d:%02d) <= iTODAY_DATE_NUM64 (%04d-%02d-%02d %02d:%02d:%02d)",
		...
		{
			CLog.MsgBox("[CastleSiege] CCastleSiege::LoadPreFixData() - FIX STATE 'CASTLESIEGE_STATE_ENDSIEGE' CAN'T BE USED");
			m_bFixCastleStateStartDate = 0;
		}
...
int CCastleSiege::LoadData(char * lpszFileName)
{
	...
	{
		CLog.MsgBox("[CastleSiege] file load error - File Name Error");
		return false;
	}

	if(m_iCastleDataLoadState != 1)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::LoadData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_1 (%d)",m_iCastleDataLoadState);
		return false;
	}
	...
	if(SMDFile == NULL)
	{
		CLog.MsgBox("[CastleSiege] file load error - fopen() : %s",lpszFileName);
		return false;
	}
	...
	while( true )
	{
		...
		while ( true )
		{
			if(type == 0)
			{
				...
				{
					CLog.LogAddC(2,"[CastleSiege] CCastleSiege::LoadData() - iCastleSiegeCycle <= 0 : %d",iCastleSiegeCycle);
					return false;
				}
				else
				{
				...
				if(!vtScheduleData.empty())
				{
					for(vector<_CS_SCHEDULE_DATA>::iterator it = vtScheduleData.begin(); it != vtScheduleData.end(); it++)
					{
						...
						if(pScheduleData.m_bIN_USE != 0)
						{
							...
							{
								CLog.LogAddC(2,"[CastleSiege] CCastleSiege::LoadData() - Same State Exist : %d",iSTATE);
								return false;
							}
							...
							if(pScheduleData.m_iSTATE > iSTATE)
							{
								...
								{
									CLog.LogAddC(2,"[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong : %d",iSTATE2_TIME);
									return false;
								}
							}
							else
			...
			else if(type == 2)
			{
				...
				if(iNPC_NUM < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_NUM < 0)");
					return false;
				}

				if(iNPC_INDEX < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_INDEX < 0)");
					return false;
				}

				if(iNPC_SIDE < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_SIDE < 0)");
					return false;
				}

				if(iNPC_DF_LEVEL < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_DF_LEVEL < 0)");
					return false;
				}

				if(iNPC_RG_LEVEL < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_RG_LEVEL < 0)");
					return false;
				}

				if(iNPC_MAXHP < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_MAXHP < 0)");
					return false;
				}

				if(iNPC_SX < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_SX < 0)");
					return false;
				}

				if(iNPC_SY < 0)
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Data Fail (iNPC_SY < 0)");
					return false;
				}
				...
				{
					CLog.MsgBox("[CastleSiege] CCastleSiege::LoadData() - Same Npc Key Exist : NUM:%d-IDX:%d",iNPC_NUM,iNPC_INDEX);
					return false;
				}
	...
	{
		for(unsigned int iIDX = 0; iIDX < m_vtScheduleData.size(); iIDX++)
		{
			if(iIDX+1 < m_vtScheduleData.size())
			{
				...
				if(iIDX_RESULT < 0)
				{
					CLog.LogAddC(2,"[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong (sort fail) : %d-%d",
...
int CCastleSiege::DataRequest()
{
	...
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::DataRequest() - m_bFileDataLoadOK == FALSE");
		return false;
	}

	if(m_iMapSvrGroup < 0)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::DataRequest() - m_iMapSvrGroup < 0");
		return false;
	}

	if(m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::DataRequest() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",m_iCastleDataLoadState);
		return false;
	}
	...
	if(m_dwDbDataLoadTickCount == 0)
	{
		...
		CLog.LogAddC(3,"[CastleSiege] CCastleSiege::DataRequest() - REQUEST DATA FIRST");
	}
	else if(GetTickCount() - m_dwDbDataLoadTickCount > 3000)
	{
		bDbRequestData = TRUE;
		m_dwDbDataLoadTickCount = GetTickCount();
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::DataRequest() - REQUEST DATA AGAIN");
	}
...
int CCastleSiege::Init()
{
	...
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::Init() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4 (%d)",m_iCastleDataLoadState);
		CLog.MsgBox("[CastleSiege] CCastleSiege::Init() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4 (%d)",m_iCastleDataLoadState);
		return false;
	}

	if(m_bFileDataLoadOK == 0 || m_bDbDataLoadOK == 0)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::Init() - Data Load Fail (FILE:%d, DB:%d)",
			m_bFileDataLoadOK,m_bDbDataLoadOK);
		CLog.MsgBox("[CastleSiege] CCastleSiege::Init() - Data Load Fail (FILE:%d, DB:%d)",
			m_bFileDataLoadOK,m_bDbDataLoadOK);
		return false;
	}

	CLog.LogAdd("[CastleSiege] CCastleSiege::Init() - START",m_bFileDataLoadOK,m_bDbDataLoadOK);
	...
	if(CheckSync() == FALSE)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::Init() - CheckSync() == FALSE");
		CLog.MsgBox("[CastleSiege] CCastleSiege::Init() - CheckSync() == FALSE");
		return false;
	}
...
void CCastleSiege::CreateAllCsGateLever()
{
	...
	{
		for(vector<_CS_NPC_DATA>::iterator it = m_vtNpcData.begin(); it != m_vtNpcData.end(); it++)
		{
			...
			if(pNpcData.m_iNPC_NUM == 277)
			{
				...
				else
				{
					...
					CLog.LogAddC(2,"[CastleSiege] CCastleSiege::CreateAllCsGateLever() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
				}
			}
		}
	}
...
int CCastleSiege::CheckSync()
{
	if(m_bFileDataLoadOK == FALSE)
	{
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::CheckSync() - m_bFileDataLoadOK == FALSE");
		return false;
	}
	...
	m_iCS_TICK_COUNT = GetTickCount();
	 ...
	 {
		 m_bFixCastleCycleStartDate = 0;
		 CLog.MsgBox("[CastleSiege] CCastleSiege::CheckSync() - iEVENT_END_DATE_NUM (%04d-%02d-%02d) <= iTODAY_DATE_NUM (%04d-%02d-%02d)",
	 ...
	 if(m_bFixCastleStateStartDate != 0)
	 {
		 ...
		 {
			 CLog.MsgBox("[CastleSiege] CCastleSiege::CheckSync() - FIXED STATE:%d NOT FOUND IN SCHEDULE (P.S.> Check File 'MuCastleData.dat', 'commonserver.cfg')",m_iFixCastleSpecificState);
			 m_bFixCastleStateStartDate = 0;
		 }
	 }
	 ...
	 else
	 {
		...
		{
			CLog.MsgBox("[CastleSiege] CCastleSiege::CheckSync() - bStateSetted == FALSE  START_DATE (%04d-%02d-%02d), END_DATE (%04d-%02d-%02d)",
				...
				if(pNpcData.m_iNPC_NUM == 277)
				{
					...
					if(bCreateLever != FALSE)
					{
						...
						if(OBJMAX_RANGE(iLeverIndex))
						{
							...
							if(bRETVAL == FALSE)
							{
								CLog.LogAddC(2,"[CastleSiege] CCastleSiege::CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
							}
						}
						else
						{
...
{
	CLog.LogAdd("SET SetState_IDLE_1()");
...
{
	CLog.LogAdd("SET SetState_REGSIEGE()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_REGSIEGE() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_IDLE_2()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_IDLE_2() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_REGMARK()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_REGMARK() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_IDLE_3()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_IDLE_3() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_NOTIFY()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_NOTIFY() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_READYSIEGE()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_READYSIEGE() - iGAP_SEC < 0");
		}
...
{
	CLog.LogAdd("SET SetState_STARTSIEGE()");
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		{
			m_bDoRun = 0;
			CLog.MsgBox("[CastleSiege] CCastleSiege::SetState_STARTSIEGE() - iGAP_SEC < 0");
		}
	...
	if(CheckAttackGuildExist() == FALSE)
	{
		...
		CLog.LogAddC(2,"[CastleSiege] CCastleSiege::SetState_STARTSIEGE() - CheckAttackGuildExist() == FALSE");
	}
	else
	{
		StoreDbNpc();
...
{
	CLog.LogAdd("SET SetState_ENDSIEGE()");
...
{
	CLog.LogAdd("SET SetState_ENDCYCLE()");
	...
	CLog.LogAdd("[CastleSiege] State - End Cycle Schedule : (%d-%d-%d(%d:%d:%d)) (%d-%d-%d(%d:%d:%d))",
	...
	CLog.LogAdd("[CastleSiege] State - End Cycle : Date-Changing Info (%d-%d-%d) (%d-%d-%d)",
...
int CCastleSiege::GetStateGapSec(int iCastleSiegeState)
{
	...
	for(vector<_CS_SCHEDULE_DATA>::iterator it = m_vtScheduleData.begin(); it != m_vtScheduleData.end(); it++)
	{
...
int CCastleSiege::CheckCastleSiegeResult()
{
	...
	CLog.LogAdd("[CastleSiege] Castle Final Winner Guild : %s",(strcmp(m_szCastleOwnerGuild,"") == FALSE) ? "NONE" : m_szCastleOwnerGuild);
...
{
	CLog.LogAdd("[CastleSiege] CCastleSiege::StoreDbNpc() - << START >>");
	...
	while(it != m_vtNpcData.end())
	{
			...
			&& gObj[pNpcData.m_iNPC_OBJINDEX].Life > 0)
		{
			...
			CLog.LogAdd("[CastleSiege] CCastleSiege::StoreDbNpc() NPC INFO (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)",
	...
	CLog.LogAdd("[CastleSiege] CCastleSiege::StoreDbNpc() - << END >>");
}
...
void CCastleSiege::SetTaxRate(int iTaxType, int iTaxRate)
{
	...
	CLog.LogAdd("[CastleSiege] Tax Rate Setted - Type:%d, Rate:%d", iTaxType, iTaxRate);
}
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( (NpcData.m_iNPC_NUM == iNpcType) && (NpcData.m_iNPC_INDEX == iNpcIndex) )
		{
				...
				if( gObj[NpcData.m_iNPC_OBJINDEX].Class == iNpcType )
				{
					if( gObj[NpcData.m_iNPC_INDEX].Live == PLAYER_CONNECTED )
					{
						if( gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal )
						{
							...
							{
								CLog.LogAdd("[CastleSiege] CCastleSiege::AddDbNPC() ERROR - DB NPC EXIST (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)",
									NpcData.m_iNPC_NUM, 
			...
			if( iObjIndex >= 0 )
			{
				...
				if( NpcData.m_iNPC_NUM == 277 )
				{
					...
					if( iGateLevelerIndex != 0 )
					{
						...
						if( OBJMAX_RANGE(iGateLevelerIndex) )
						{
							...
							if( iLinkIndex == 0 )
							{
								::CLog.LogAddC(2, "[CastleSiege] CCastleSiege::CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",
									NpcData.m_iNPC_INDEX);
							}
						}
						else
				...
				{
					CLog.LogAdd("[CastleSiege][CreateDBNPC] (Index:%d, Num:%d, DF:%d, RG:%d, MaxHP:%d, HP:%d )",
...
BOOL CCastleSiege::UpgradeDbNPC(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex)
{
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::UpgradeDbNPC() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d", iNpcUpIndex);
		return FALSE;
	}
	...
	if( bIsLive == FALSE )
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::UpgradeDbNPC() ERROR - bIsLive == FALSE", iNpcUpIndex);
		return FALSE;
	}
...
BOOL CCastleSiege::FirstCreateDbNPC()
{
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::FirstCreateDbNPC() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4");
		return FALSE;
	}
...
BOOL CCastleSiege::SetCastleNpcData(CSP_CSINITDATA* lpMsg, int iCOUNT)
{
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::SetCastleInitData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3");
		return FALSE;
	}
...
BOOL CCastleSiege::SetCastleInitData(CSP_ANS_CSINITDATA* lpMsg)
{
	...
	CLog.LogAdd("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Start Date (%d-%d-%d)",
		m_tmStartDate.wYear, m_tmStartDate.wMonth, m_tmStartDate.wDay);
	
	CLog.LogAdd("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Date (%d-%d-%d)",
	...
	CLog.LogAdd("[CastleSiege] CCastleSiege::LoadData() - Siege Schedule End Date (%d-%d-%d(%d:%d:%d)",
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::SetCastleInitData() - iSTART_DATE_NUM > iEND_DATE_NUM");
		return FALSE;
	}
	return TRUE;
}
...
void CCastleSiege::MakeCsTotalGuildInfo(CSP_CSGUILDUNIONINFO* lpMsg, int iCOUNT)
{
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::MakeCsTotalGuildInfo() ERROR - m_bCsBasicGuildInfoLoadOK == FALSE");
		return;
	}
	if( iCOUNT <= 0 )
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::MakeCsTotalGuildInfo() ERROR - iCOUNT <= 0 : %d", iCOUNT);
		return;
	}
...
void CCastleSiege::SetCsTotalGuildInfo(CSP_CSLOADTOTALGUILDINFO* lpMsg, int iCOUNT)
{
	...
	if( iCOUNT <= 0 )
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::SetCsTotalGuildInfo() ERROR - iCOUNT <= 0 : %d",
			iCOUNT);
	...
	for( ; it2 != m_mapCsTotalGuildInfo.end(); it2++ )
	{
		...
		CLog.LogAdd("[CastleSiege] Siege Guild List Load - GNAME:%s, CSGUID:%d, INVOLVED:%d",
...
void CCastleSiege::Run()
{
	...
	if( (GetTickCount() - g_iLastSiegeUpdate) > 3000 )
	{
		...
		if( iLeftSiegeDate >= 0 )
		{
			...
			CLog.LogAddC(4, "LEFT : SEC(%d), SIEGE-DAY(%04d-%02d-%02d %02d:%02d:%02d)",
		...
		else
		{
			...
			case CASTLESIEGE_LEFTTIME_ONSIGE:
				CLog.LogAddC(4, "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_ONSIGE");
				break;
			case CASTLESIEGE_LEFTTIME_ENDSIEGE:
				CLog.LogAddC(4, "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_ENDSIEGE");
				break;
			case CASTLESIEGE_LEFTTIME_NOTRUNNING:
				CLog.LogAddC(4, "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_NOTRUNNING");
				break;
			}
		}
	}
...
int CCastleSiege::CalcCastleLeftSiegeDate()
{
	...
	if( iGAP_SEC >= 0 )
	{
		...
		if( iGAP_SEC == 0 )
		{
			CLog.LogAddC(2, "[CastleSiege] CCastleSiege::CalcCastleLeftSiegeDate() - iGAP_SEC == 0");
		}
...
void CCastleSiege::ProcState_IDLE_1()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_IDLE_1()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_REGSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_REGSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_IDLE_2()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		this->m_iCS_REMAIN_MSEC -= iTICK_MSEC;
		this->m_iCS_TICK_COUNT = GetTickCount();
		CLog.LogAddC(3, "RUN ProcState_IDLE_2()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_REGMARK()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_REGMARK()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_IDLE_3()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_IDLE_3()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_NOTIFY()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_NOTIFY()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_READYSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_READYSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_STARTSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_STARTSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_ENDSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_ENDSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_ENDCYCLE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		CLog.LogAddC(3, "RUN ProcState_ENDCYCLE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
			...
			if( (NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) && NpcData.m_iNPC_OBJINDEX != OBJ_EMPTY )
			{
					...
					( gObj[NpcData.m_iNPC_OBJINDEX].Class == 277 || gObj[NpcData.m_iNPC_OBJINDEX].Class == 283 ) && gObj[NpcData.m_iNPC_OBJINDEX].Live == 0) )
				{
					...
					case 277:
						...
						CLog.LogAdd("[CastleSiege] Castle Gate is Broken (CLS:%d, IDX:%d, X:%d, Y:%d)",
					...
					case 283:
						...
						CLog.LogAdd("[CastleSiege] Defend Statue is Broken (CLS:%d, IDX:%d, X:%d, Y:%d)",
							NpcData.m_iNPC_NUM, NpcData.m_iNPC_INDEX, NpcData.m_iNPC_SX, NpcData.m_iNPC_SY);
						break;
					}
		...
		(gObj[this->m_iCastleSwitchAccessUser2].m_btCsJoinSide >= 2 ) )
	{
		if( gObj[this->m_iCastleCrownAccessUser].m_bCsGuildInvolved == TRUE )
		{
				...
				(gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide == gObj[this->m_iCastleSwitchAccessUser2].m_btCsJoinSide) )
			{
				...
				{
					CLog.LogAdd("[CastleSiege] Castle Winner Has been Changed (GUILD:%s)",
...
void CCastleSiege::ChangeWinnerGuild(int iCsJoinSide)
{
	...
	CLog.LogAdd("[CastleSiege] Castle Winner Guild Change From '%s' To '%s'", szGuildFrom, szGuildTo);
}
...
int CCastleSiege::DelGemOfDefend(int iIndex, int iNeedGemOfDefend)
{
	...
	for(int x = 0; x < INVENTORY_SIZE; x++ )
	{
		if( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if( gObj[iIndex].pInventory[x].m_Type == ITEMGET(0,479) )
			{
				CLog.LogAdd("[CastleSiege] [%s][%s] Deleting Gem of Defend (GUILD:%s) - Serial:%d",
			...
			if( NpcData.m_iCsNpcExistVal == iMonsterExistVal && NpcData.m_bIN_USE == TRUE )
			{
				if( NpcData.m_iNPC_LIVE > 0 )
				{
					...
					if( NpcData.m_iNPC_NUM == 277 )
					{
						...
						CLog.LogAdd("[CastleSiege][ State:%d ] Delete Castle Gate ( CLS:%d, IDX:%d, X:%d. Y:%d )",
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	if( m_bCsTotalGuildInfoLoadOK == FALSE )
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::SaveCsTotalGuildInfo() ERROR - m_bCsTotalGuildInfoLoadOK == FALSE");
		return;
	}
	char cBUFFER[2684];
	...
	for( int iSize = m_mapCsTotalGuildInfo.size(); it2 != m_mapCsTotalGuildInfo.end(); it2++ )
	{
		...
		CLog.LogAdd("[CastleSiege] Siege Guild List Save - GNAME:%s, CSGUID:%d, INVOLVED:%d",
...
void CCastleSiege::NotifyCrownSwitchInfo(int iIndex) /*00578550*/
{
	if( gObjIsConnected(iIndex) == FALSE )
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::NotifyCrownSwitchInfo() - iCrownSwitchIndex is Invalid");
		...
		case 0:
			{
				...
				{
					MsgOutput(iIndex, "¼ö¼ºÃø ±æµåº¯°æ ½ÇÆÐ");
				}
				else
				{
					if( strcmp(szNewOwnerGuild, "") != 0 )
					{
						...
						MsgOutput(iIndex, "¼ö¼ºÃø ±æµåº¯°æ: %s", this->m_szCastleOwnerGuild);
					}
					else
					{
		...
		case 1:
			{
				...
				if( this->CheckSync() == FALSE )
				{
					CLog.LogAddC(2, "[CastleSiege] CCastleSiege::Init() - CheckSync() == FALSE");
					CLog.MsgBox("[CastleSiege] CCastleSiege::Init() - CheckSync() == FALSE");
					MsgOutput(iIndex, "»óÅÂº¯°æ ½ÇÆÐ - °ø¼ºÂü¿©µî·Ï");
				}
				else
				{
					this->m_bDoRun = TRUE;
					...
					MsgOutput(iIndex, "»óÅÂº¯°æ ¼º°ø - °ø¼ºÂü¿©µî·Ï");
				}
			}
			break;
		case 2:
			{
				this->SetState(CASTLESIEGE_STATE_IDLE_2, 1);
				MsgOutput(iIndex, "»óÅÂº¯°æ ¼º°ø - °ø¼º¹®Àåµî·Ï");
			}
			break;
		case 3:
			{
				...
				this->SetState(CASTLESIEGE_STATE_IDLE_3, 1);
				MsgOutput(iIndex, "»óÅÂº¯°æ ¼º°ø - °ø¼º¼±Æ÷");
			}
			break;
		case 4:
			{
				this->SetState(CASTLESIEGE_STATE_READYSIEGE, 1);
				MsgOutput(iIndex, "»óÅÂº¯°æ ¼º°ø - °ø¼º½ÃÀÛ");		
			}
			break;
		...
		case 5:
			{
				if( this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE )
				{
					...
					MsgOutput(iIndex, "»óÅÂº¯°æ ¼º°ø - °ø¼ºÁ¾·á");
				}
				else
				{
					MsgOutput(iIndex, "»óÅÂº¯°æ ½ÇÆÐ - °ø¼ºÁ¾·á");
				}
			}
			break;
		case 6:
			{
				if( strcmp(this->m_szCastleOwnerGuild, "") == 0 )
				{
					MsgOutput(iIndex, "ÇöÀç ¼ö¼ºÃø ¾øÀ½");
				}
				else
				{
					MsgOutput(iIndex, "ÇöÀç ¼ö¼ºÃø ±æµå  - %s", this->m_szCastleOwnerGuild);
				}
			}
			break;
		case 7:	/* Get Siege State Time Left */
			{
				if( this->m_bDoRun == FALSE )
				{
					MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - ¾Ë ¼ö ¾øÀ½");
				}
				else
				{
					switch( this->m_iCastleSiegeState )
					...
					case CASTLESIEGE_STATE_NONE:
						...
						break;
					case CASTLESIEGE_STATE_IDLE_1:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼º ÁÖ±â Á÷Àü (½ÃÀÛ¾ÊµÊ)");
						break;
					case CASTLESIEGE_STATE_REGSIEGE:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü µî·Ï±â°£ : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60, 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_IDLE_2:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü µî·ÏÁ÷ÈÄ (ÈÞ½Ä±â°£) : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_REGMARK:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - ¹®Àå µî·Ï±â°£ : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_IDLE_3:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - ¹®Àå µî·ÏÁ÷ÈÄ  (ÈÞ½Ä±â°£) : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_NOTIFY:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü Âü¿©±æµå ¹ßÇ¥±â°£ : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_READYSIEGE:
						...
						break;
					case CASTLESIEGE_STATE_STARTSIEGE:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü ÁØºñ±â°£ : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_ENDSIEGE:
						MsgOutput(iIndex, "ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü ÀüÀï Áß : %d½Ã°£:%dºÐ:%dÃÊ ³²À½",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
		...
		case 8:
			{
				...
				if( this->m_iCastleSiegeState != CASTLESIEGE_STATE_STARTSIEGE )
				{
					MsgOutput(iIndex, "°ø¼ö±³Ã¼ ½ÇÆÐ...ÇöÀç °ø¼º»óÅÂ - °ø¼ºÀü ÁÖ±âÁ¾·á");
				}
				...
				else
				{
					...
					if( iResult == 1 )
					{
						...
						{
							CLog.LogAdd("[CastleSiege] Castle Winner Has been Changed - GM (GUILD:%s)",
								szGuildName);
...
void CCastleSiege::NotifyCrownState(BYTE btState)
{
	...
	{
		CLog.LogAddC(2, "[CastleSiege] CCastleSiege::NotifyCrownState() - m_iCrownIndex isn't Valid");
		return;
	}
	...
	CLog.LogAdd("[CastleSiege] Crown State Changed (%d)", btState); // dor na mão d aporra -.-'
}
