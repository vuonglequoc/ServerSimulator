...
#include "../Header Files/DevilSquareGround.h"
		...
		if ( this->m_DevilSquareBossMonsterInfo[i].m_Type != (WORD)-1 )
		{
			if ( currtime == this->m_DevilSquareBossMonsterInfo[i].m_RegenStartTime )
			{
				...
				if ( iMonIndex >= 0 )
				{
					...
					CLog.LogAdd("[DevilSquare] Boss %d %d %d create ", map, x, y);
					...
					gObj[iMonIndex].StartX = static_cast<BYTE>(gObj[iMonIndex].X);
					gObj[iMonIndex].StartY = static_cast<BYTE>(gObj[iMonIndex].Y);
...
void CDevilSquareGround::SendScore()
{
	...
	C1HeadSet((LPBYTE)&this->m_DevilSquareScoreInfoTOP10, 0x93, iSize);
	count = 1;

	Itor = this->m_DevilSquareRankList.begin();

	CLog.LogAdd("[DevilSquare] Rank [%d]", this->m_iIndex);
	...
	for ( ; Itor != this->m_DevilSquareRankList.end(); Itor++ )
	{
		...
		CLog.LogAdd("Rank :[%d] : [%s][%s][%d][%d][%d]", count, (*(Itor))->AccountID, (*(Itor))->Name, (*(Itor))->m_nEventMoney, (*(Itor))->m_nEventExp, (*(Itor))->m_nEventScore);
		...
		gSendProto.DataSend((*(Itor))->m_Index, (BYTE *)&this->m_DevilSquareScoreInfoTOP10 , iSize);
