...
//	GS-N	2.00.05	JPN	0x00563910	-	Completed
//////////////////////////////////////////////////////////////////////
...
BOOL TMonsterAIUnit::LoadData(const char* lpszFileName)
{
	TMonsterAIUnit::s_bDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, ""))
	{
		CLog.MsgBox("[Monster AI Unit] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Monster AI Unit] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster AI Unit] - UnitNumber(%d) Error (%s) File. ", 
					...
					{
						CLog.MsgBox("[Monster AI Unit] - AutomatNumber(%d) Error (%s) File. ", 
		...
		CLog.LogAddC(TColor.Red(), "[Monster AI Unit] - %s file is Loaded", lpszFileName);

		TMonsterAIUnit::s_bDataLoad = TRUE;
	}
	catch(DWORD)
	{
		CLog.MsgBox("[Monster AI Unit] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
TMonsterAIUnit * TMonsterAIUnit::FindAIUnit(int iUnitNumber)
{
	if ( iUnitNumber < 0 || iUnitNumber >= MAX_MONSTER_AI_UNIT )
	{
		CLog.LogAdd("[Monster AI Unit] FindAIUnit() Error - (UnitNumber=%d) ",
			iUnitNumber);
