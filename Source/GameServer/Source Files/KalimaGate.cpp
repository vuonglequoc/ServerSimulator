...
//	GS-N	2.00.05	JPN	0x004102D0	-	Completed
//	GS-CS	2.00.05	JPN	0x0041XXXX	-	Completed
#include "../Header Files/stdafx.h"
#include "../Header Files/KalimaGate.h"
...
BOOL CKalimaGate::CreateKalimaGate(int iIndex, BYTE btLevel, BYTE cTX, BYTE cTY)
{
	...
	__try
	{
		...
		CLog.LogAdd("[Kalima] [%s][%s] Try to Create Kalima Gate", gObj[iIndex].AccountID, gObj[iIndex].Name);
		...
		if ( gUser.ChaosCastleMapRange(gObj[iIndex].MapNumber) == true )
		{
			return false;
		}
		...
		{
			CLog.LogAdd("[Kalima] [%s][%s] Failed to Summon Kalima Gate - Gate Level Check Error : %d",
		...
		if ( (btMapAttr &1) != 0 )
		{
			CLog.LogAdd("[Kalima] [%s][%s] Failed to Summon Kalima Gate - Called in Saftey Area (Map:%d, X:%d, Y:%d)",
		...
		if ( gObj[iIndex].m_cKalimaGateExist == TRUE )
		{
			...
			CLog.LogAdd("[Kalima] [%s][%s] Failed to Summon Kalima Gate - Already Have Gate (SummonIndex:%d)",
		...
		if ( btMapAttr != 0 )
		{
			...
			{
			CLog.LogAdd("[Kalima] [%s][%s] Failed to Summon Kalima Gate - Failed to get Location (MapNumber:%d, X:%d, Y:%d)",
		...
		{
			CLog.LogAdd("[Kalima] [%s][%s] Failed to Summon Kalima Gate - Uable to Summon in this Map (MapNumber:%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].MapNumber);
			return false;
		}
		...
		if ( iMonsterIndex >= 0 )
		{
			...
			if ( lpMATTR == NULL )
			{
				...
				CLog.LogAdd("[Kalima] [%s][%s] Kalima Gate Vanished - lpMATTR == NULL (SummonIndex:%d)",
			...
			CLog.LogAdd("[Kalima] [%s][%s] Create Kalima Gate Successfuly (Live:%d, GateTick:%u, DieRegen:%d, Map:%d, X:%d, Y:%d, SummonIndex:%d, OwnerIndex:%d, GateNo.:%d)",
...
void CKalimaGate::KalimaGateAct(int iIndex)
{
	...
	{
		CLog.LogAdd("[Kalima] Kalima Gate Vanished - Summoner Vanished (SummonIndex:%d, EnterCount:%d)",
	...
	if ( gObj[gObj[iIndex].m_RecallMon].DieRegen > 0 )
	{
		...
		CLog.LogAdd("[Kalima] [%s][%s] Kalima Gate Vanished - Summoner Died (SummonIndex:%d, EnterCount:%d)",
	...
	if ( gObj[iIndex].m_cKalimaGateEnterCount >= MAX_KALIMA_ENTER )
	{
		...
		CLog.LogAdd("[Kalima] [%s][%s] Kalima Gate Vanished - Enter Count Over (SummonIndex:%d, EnterCount:%d)",
			lpObjCallOwner->AccountID, lpObjCallOwner->Name,
			iIndex, gObj[iIndex].m_cKalimaGateEnterCount );
		this->DeleteKalimaGate(iIndex, gObj[iIndex].m_RecallMon);
		return;
	...
	if ( dwTickCount > lpObj->RegenTime + KALIMA_GATE_TIMEOUT )
	{
		...
		CLog.LogAdd("[Kalima] [%s][%s] Kalima Gate Vanished - Time Out (SummonIndex:%d, EnterCount:%d)",
			lpObjCallOwner->AccountID, lpObjCallOwner->Name,
			iIndex, gObj[iIndex].m_cKalimaGateEnterCount );
						...
						if ( abs(lpObj->X - gObj[tObjNum].X) < 2 )
						{
							if ( lpObj->m_RecallMon == tObjNum || (gObj[tObjNum].PartyNumber != -1 && lpObjCallOwner->PartyNumber == gObj[tObjNum].PartyNumber ))
							{
								...
								if ( iOverLevel >= iMoveMapLevel )
								{
									...
									{
										CLog.LogAdd("[Kalima] [%s][%s] - [%s][%s] move fail (MapNumber:%d)", 
											lpObjCallOwner->AccountID, lpObjCallOwner->Name,
											gObj[tObjNum].AccountID, gObj[tObjNum].Name, iMapNumber);
									}
									else
									{
										CLog.LogAdd("[Kalima] [%s][%s] - [%s][%s] Transported by Kalima Gate (SummonIndex:%d, GateNo.:%d, MapNumber:%d)",
											lpObjCallOwner->AccountID, lpObjCallOwner->Name, 
...
{
	if( gObj[iIndex].Class = CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_FIGHTER)
	{
...
BOOL CKalimaGate::DeleteKalimaGate(int iKalimaGateIndex, int iCallOwnerIndex)
{
	...
	__try
	{
		...
		{
			CLog.LogAdd("[Kalima] DeleteKalimaGate() - out of Index (iKalimaGateIndex:%d, iCallOwnerIndex:%d)",
...
BOOL CKalimaGate::DeleteKalimaGate(int iCallOwnerIndex)
{
	...
	{	
		CLog.LogAdd("[Kalima] DeleteKalimaGate() - out of Index (iCallOwnerIndex:%d)",
			iCallOwnerIndex);
		return false;
	}
...
void CKalimaGate::KalimaGateAct2(int iIndex)
{
	...
	{
		CLog.LogAdd("[Kalima] Kalima Gate Vanished (NextMap) - Summoner Vanished (SummonIndex:%d, EnterCount:%d)",
			iIndex, gObj[iIndex].m_cKalimaGateEnterCount);
	...
	if ( dwTickCount > (lpObj->RegenTime+60000) )
	{
		...
		CLog.LogAdd("[Kalima] [%s][%s] Kalima Gate Vanished (NextMap) - Time Out (SummonIndex:%d, EnterCount:%d)",
			lpObjCallOwner->AccountID, lpObjCallOwner->Name, iIndex, gObj[iIndex].m_cKalimaGateEnterCount);
					...
					if ( abs(lpObj->Y - gObj[tObjNum].Y) < 2 )
					{
						if ( abs(lpObj->X - gObj[tObjNum].X) < 2 )
						{
							if ( lpObj->m_RecallMon == tObjNum || (gObj[tObjNum].PartyNumber != -1 && lpObjCallOwner->PartyNumber == gObj[tObjNum].PartyNumber ))
							{
								...
								{
									CLog.LogAdd("[Kalima] [%s][%s] - [%s][%s] Transported by Kalima Gate (NextMap) (SummonIndex:%d, GateNo.:%d, MapNumber:%d)",
										lpObjCallOwner->AccountID, lpObjCallOwner->Name,
										gObj[tObjNum].AccountID, gObj[tObjNum].Name,
