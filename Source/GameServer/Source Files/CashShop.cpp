...
CCashShop::~CCashShop()
{
	for (std::map<int, CASHSHOP_ITEM_STATUS*>::iterator MapItor = MapCashItemStatus.begin(); MapItor != MapCashItemStatus.end(); ++MapItor)
	{
		CASHSHOP_ITEM_STATUS* Value = (*MapItor).second;
		delete Value;
	}
}
...
void CCashShop::CashShopOptioNReload()
{
	...
	CLog.LogAdd("[CashShop][OptionLoad] - CashShop Option Load Begin");

	LoadShopOption("..\\Data\\CashShopOption.dat");
	Load("..\\Data\\CashShopList.txt");
	GSReqBranchItemList();
	GSReqCashItemList();
	GSReqPackageItemList();

	CLog.LogAdd("[CashShop][OptionLoad] - CashShop Option Load finish");
...
void CCashShop::Load(const char* pchFilename)
{
	SMDToken Token;
	...
	if ( SMDFile == NULL )
	{
		CLog.MsgBox("[CashItemList] Cash Item List load failed. [%s]", pchFilename);
		return;
	}
	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			if ( iType == 1 )
			{
				...
				CLog.LogAdd("[CashShop][Load Cash Item List] - Add List - (%d/%d) Level:%d, Dur:%d, Skill:%d, Luck:%d, Add:%d, Ex:%d, X:%d, Y:%d, Scale:%d",
	...
	CLog.LogAdd("[CashShop][Load Cash Item List] - Complete! - Total:%d",
	...
	if ( g_bConnectShopServer == TRUE && g_bShopServerConnectState == FALSE)
	{
		...
		{
			CLog.MsgBox("[CashShop] Shop Server Connect Failed!!");
		}
	}
}
...
void CCashShop::LoadShopOption(const char* pchFilename)
{
	CLog.LogAdd("[CashShop][ShopOption] - Shop Option Reload '%s'", pchFilename);
...
void CCashShop::LoadTestScript(LPSTR pchFilename)
{
	...
	if ( SMDFile == NULL )
	{
		CLog.MsgBox("Error","[CashItemList] Cash Item List load failed. [%s]", pchFilename);
		return;
	}
...
{
	if ( g_ShopServerClient.CreateSocket(gMainApp.m_hWnd) == FALSE )
		return FALSE;

	if ( !g_ShopServerClient.SetProtocolCore(ShopServerProtocolCore) )
		return FALSE;

	CCashShop::m_lpOldProc = (WNDPROC)SetWindowLong(gMainApp.m_hWnd, GWL_WNDPROC, (LONG)CCashShop::ParentWndProc);
	...
	switch( iMessage )
	{
		case 0x41E:
			switch ( lParam & 0xFFFF & 0xFFFF )
			{
				...
				case 16:
					CLog.LogAdd("[ShopServer] - Connect To Shop Server!!");
					g_bShopServerConnectState = TRUE;
					return 0;

				case 32:
					g_ShopServerClient.Close();
					CLog.LogAddC(TColor.Red(), "[ShopServer] - Disconnect From Shop Server!!");
...
void CCashShop::CheckShopServerConnectState()
{
	...
	if ( (GetTickCount()-this->dwCheckShopServerConnectStatePeriod) > 10000 )
	{
		...
		if ( g_bShopServerConnectState == FALSE )
		{
			...
			if ( g_bShopServerConnectState == FALSE )
			{
				CLog.LogAddC(TColor.Red(), "[CashShop][ShopServer] - Disconnect From Shop Server!!");
			}
		}
	}
}
...
BOOL CCashShop::SetItemInfoFromShop(protocol::MSG_STOG_ITEM_LIST_ANS * lpMsg)
{
	...
	for (int i=0;i<iItemCount;i++)
	{
		...
		{
			CLog.LogAdd("[CashShop] Invalid Item Info from ShopServer (GUID:%d,Category:%d,ItemCode:%d,Price:%d)",
		...
		{
			CLog.LogAdd("[CashShop] Can not Find Item Status (Code : %d)", iItemCode);
			continue;
		}
...
void CCashShop::MakeItemList(CASHSHOP_ITEM_STATUS * lpItemStatus, sellItem * lpItemSellInfo)
{
	...
	for ( iIndex=0;iIndex<iItemCount;iIndex++)
	{
		...
		this->CashItemList[iCategory][iItemIndex+iIndex].btItemUsedType = (BYTE)lpItemSellInfo->dwUseType;
		...
		{
			this->CashItemList[iCategory][iItemIndex+iIndex].btItemDuration = (BYTE)lpItemSellInfo->itemPrice[iIndex].dwAmount;
		}
		else
		{
		...
		this->CashItemList[iCategory][iItemIndex+iIndex].btItemSaleRatio = (BYTE)lpItemSellInfo->itemPrice[iIndex].dwSellRate;
		this->CashItemList[iCategory][iItemIndex+iIndex].wItemPrice = (WORD)lpItemSellInfo->itemPrice[iIndex].dwPrice;
		...
		//	lpItemStatus->btSkillOption, lpItemStatus->btLuckOption, lpItemStatus->btAddOption, lpItemStatus->btExOption, 0, 0);
		...
		this->CashItemListCompress[iCategory][iItemIndex+iIndex].btItemSaleRatio = (BYTE)lpItemSellInfo->itemPrice[iIndex].dwSellRate;
		this->CashItemListCompress[iCategory][iItemIndex+iIndex].wItemPrice = (WORD)lpItemSellInfo->itemPrice[iIndex].dwPrice;
		...
		CLog.LogAdd("[CashShop][ShopList] (Category:%d, Guid:%d) ItemInfo(Type : %d, Index : %d, Name : %s, Price : %d(%d) Amount : %d, UseTime : %d, SPOP:%d ",
...
void CCashShop::MakeItemListProtocol()
{
	...
	for ( iCategory=0;iCategory < MAX_CASH_SHOP_CATEGORY;iCategory++)
	{
		...
		for ( int i=0;i<iAddItemCount;i++)
		{
			...
			C1SubHeadSet((LPBYTE)&this->CashItemProtocol[iCategory][iPageIndex], 0xD2, 0x06, sizeof(PMSG_ANS_CASHITEMLIST));
		}
	}
}
...
BOOL CCashShop::CGCashShopOpen(LPOBJ lpObj, PMSG_REQ_CASHSHOPOPEN * lpMsg)
{
	...
	{
		CLog.LogAddC(TColor.Red(), lMsg.Get(MSGGET(1, 175)), lpObj->AccountID, lpObj->m_Index);
		return FALSE;
	}*/
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xD2, 0x02, sizeof(PMSG_ANS_CASHSHOPOPEN));

	gSendProto.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.head.size);
...
void CCashShop::GCCashPoint(LPOBJ lpObj, DWORD dwCashPoint)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xF5, 0x04, sizeof(PMSG_ANS_CASHPOINT));
	lpObj->m_wCashPoint = (WORD)dwCashPoint;
...
BOOL CCashShop::GCCashItemListSend(LPOBJ lpObj, PMSG_REQ_CASHITEMLIST *lpMsg)
{
	...
	gSendProto.DataSend(lpObj->m_Index, lpSendMsg, sizeof(PMSG_ANS_CASHITEMLIST));
...
void CCashShop::CGCashItemBuy(LPOBJ lpObj, PMSG_REQ_CASHITEM_BUY *lpMsg)
{
	...
	CLog.LogAdd("[CashShop][Buy Request] User(ID:%s, Name:%s) Item(Name:%s,Guid:%d,Category:%d,Price:%d,SaleRate:%d) Result:%d",
...
void CCashShop::GCCashItemBuyResult(LPOBJ lpObj, BYTE btResult)
{
	...
	pMsg.btResult = btResult;
	C1SubHeadSet((LPBYTE)&pMsg, 0xF5, 0x08, sizeof(PMSG_ANS_CASHITEM_BUY));
...
BOOL CCashShop::GiveBuyItemToInventory(LPOBJ lpObj, int iItemGuid)
{
	...
	ItemSerialCreateSend(lpObj->m_Index, 0xEC, (BYTE)lpObj->X, (BYTE)lpObj->Y, 
		iItemCode, (BYTE)pItem.m_Level,
		(BYTE)pItem.m_Durability, pItem.m_Option1, pItem.m_Option2, 
		pItem.m_Option3, lpObj->m_Index,
		pItem.m_NewOption, 0, pItem.m_ItemSocket);

	CLog.LogAdd("[CashShop][Buy Item Create in Inven] - User(ID:%s,Name:%s) Item(Name:%s,Code:%d,Skill:%d,Luck:%d,Add:%d,Ex(%d:%d:%d:%d:%d:%d))",
...
void CCashShop::GSReqCashItemList()
{
	...
	CLog.LogAdd("[CashShop] Request Cash Item List to ShopServer.");
...
void CCashShop::GSReqPackageItemList()
{
	...
	CLog.LogAdd("[CashShop] Request Cash Package Item List to ShopServer.");
...
{
	CLog.LogAdd("[CashShop] Receive Cash Item List from ShopServer. (%d)", aRecv->dwItemCount);
...
{
	CLog.LogAdd("[CashShop] Receive Cash Package Item List from ShopServer. (%d)", aRecv->dwPackageCount);
...
{
	CLog.LogAdd("[CashShop] Request Cash Branch Item List to ShopServer.");
...
void SGAnsBuyCashItem( protocol::MSG_STOG_BUY_ITEM_ANS* aRecv)
{
	...
	lpObj->m_wCashPoint = (WORD)aRecv->dwUserCash;
	...
	CLog.LogAdd("[CashShop][Buy Answer] User(ID:%s, Name:%s) Result:%d",
