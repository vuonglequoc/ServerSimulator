...
//	GS-N	2.00.05	JPN	0x005914B0 - Completed
//////////////////////////////////////////////////////////////////////

#include "../Header Files/stdafx.h"
#include "../Header Files/wsShopServerCli.h"
#include "../Header Files/PCSProtocol.h"
...
BOOL wsShopServerCli::Connect(LPSTR ip_addr, WORD port,DWORD WinMsgNum)
{
	...
	if ( this->m_hWnd == NULL )
	{
		CLog.LogAdd(" Connect Error %s %d", __FILE__, __LINE__);
		return FALSE;
	}
	...
	if ( addr.sin_addr.S_un.S_addr == -1 )
	{
		...
		if ( host == 0 )
		{
			CLog.LogAdd("server name not found [%s]", ip_addr);
			return FALSE;
		}
	...
	if ( nResult == -1 )
	{
		...
		CLog.LogAdd("Client WSAAsyncSelect error %d", WSAGetLastError());
		return FALSE;
	}
	this->m_SendBufLen=0;
...
BOOL wsShopServerCli::DataSend(PCHAR buf, int len)
{
	...
	while ( true )
	{
		...
		if ( nResult == -1)
		{
			if ( WSAGetLastError() != WSAEWOULDBLOCK )
			{
				CLog.LogAdd("error-L3 : %d (%s %d)", WSAGetLastError(), __FILE__, __LINE__);
				*sendbuflen=0;
				this->Close(this->m_socket );
				return FALSE;
			}
			...
			{
				CLog.LogAddC(TColor.Red(),"error-L3 : buffer error closed %d, %d", (*sendbuflen+nLeft), 819200);
				*sendbuflen = 0;
				this->Close(this->m_socket);
				return FALSE;
			}
			...
			if ( nDx >= 0 )
			{
				...
				CLog.LogAdd("error-L3 : Copy1 : nLeft:%d len:%d nDx:%d", nLeft, this->m_SendBufLen , nDx);
				return TRUE;
			}
		}
		...
		{
			CLog.LogAdd("error-L3 : send()  result is zero", WSAGetLastError());
			break;
		}
...
BOOL wsShopServerCli::FDWRITE_MsgDataSend()
{
	...
	while (*sendbuflen>0)
	{
		...
		if (nResult == -1)
		{
			if ( WSAGetLastError() != WSAEWOULDBLOCK)
			{
				CLog.LogAdd("FD_WRITE send() fail &d %d", WSAGetLastError(), *sendbuflen);
				this->Close();
				return FALSE;
			}
			else
			{
				CLog.LogAdd("FD_WRITE send() WSAEWOULDBLOCK : %d", WSAGetLastError());
				break;
			}
		}
		if (nResult <= 0)
		{
			CLog.LogAdd("send() result is zero %d", WSAGetLastError());
			break;
		}
		CLog.LogAdd("error-L3 : nDx %d m_SendBufLen %d", nDx, this->m_SendBufLen );
...
int wsShopServerCli::DataRecv()
{
	...
	if ( nResult == 0 )
	{
		CLog.LogAdd("error-L3 : closed %d - buff:%d(%d) ", WSAGetLastError(), this->m_RecvBufLen, 819200 - *recvbuflen );
		return 1;
	}
	else if (nResult == -1 )
	{
		...
		CLog.LogAdd("error-L3 : recv error %d", WSAGetLastError() );
		return 1;
	}
	...
	*recvbuflen += nResult;
	...
	while ( true )
	{
		...
		if ( size <= 0 )
		{
			CLog.LogAdd("error-L2 : size %d", size);
			return 0;
		}
