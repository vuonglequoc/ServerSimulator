...
#include "../Header Files/KanturuBattleOfMaya.h"
...
BOOL CKanturuBattleOfMaya::LoadData(const char * lpszFileName)
{
	this->m_bFileDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName , "") )
	{
		CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 2 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Exceed Max Scene (%d)",
				...
				else if ( iType == 3 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Exceed Max State Time (%d)",
				...
				else if ( iType == 4 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Exceed Maya Hand AI Change Time(%d)",
							this->m_iMayaHandAIChangeTimeCount);	// #error Change to m_iMayaHandAIChangeTimeCount
				...
				else if ( iType == 5 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Maya Hand AI Group Read Error(%d)",
				...
				else if ( iType == 6 )
				{
					...
					if ( iValue < 0 )
					{
						CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Maya IceStorm Using Rate Load Error(%d)",
							iValue);
					...
					CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] IceStorm Using Rate Load: %d",
		...
		fclose(SMDFile);
		CLog.LogAddC(2, "[ KANTURU ][ BattleOfMaya ] %s file is Loaded",
			lpszFileName);
	...
	{
		CLog.MsgBox("[ KANTURU ][ BattleOfMaya ] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> NONE",
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> STANBY1",
		this->m_iBattleOfMayaState);
	CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] State(%d) -> STANBY1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> NOTIFY1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> START1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> MAYA1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> END_MAYA1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> ENDCYCLE_MAYA1",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> STANBY2",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> START2",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> MAYA2",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> END_MAYA2",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> ENDCYCLE_MAYA2",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> STANBY3",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> START3",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> MAYA3",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> END_MAYA3",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> ENDCYCLE_MAYA3",
		this->m_iBattleOfMayaState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> END",
		this->m_iBattleOfMayaState);
	...
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] Success Value[%d]",
...
{
	CLog.LogAddC(5, "[ KANTURU ][ BattleOfMaya ] State(%d) -> ENDCYCLE",
		this->m_iBattleOfMayaState);

	CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] State(%d) -> ENDCYCLE",
		this->m_iBattleOfMayaState);
...
void CKanturuBattleOfMaya::ProcState_START1()
{
	if ( g_KanturuMonsterMng.IsExistAliveMonster() == FALSE )
	{
		...
		CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] Success!! - DetailState:%d",
			this->m_iBattleOfMayaState);
	...
	if ( this->GetBattleSceneState() != -1 )
	{
		...
		if ( this->m_BattleTimeInfo[this->m_iSceneState].IsTimeOut() == TRUE )
		{
			if ( this->GetBattleOfMayaState() != KANTURU_MAYA_END )
			{
				...
				CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] Fail!! TimeOut - DetailState:%d",
					this->m_iBattleOfMayaState);
...
void CKanturuBattleOfMaya::CheckMayaBattleUser()
{
		 ...
		 this->GetBattleOfMayaState() == KANTURU_MAYA3 )
	{
		if ( g_KanturuBattleUserMng.IsEmpty() == TRUE )
		{
			...
			CLog.LogAdd("[ KANTURU ][ BattleOfMaya ] Fail!! All Battle User Die - DetailState:%d",
				this->m_iBattleOfMayaState);
