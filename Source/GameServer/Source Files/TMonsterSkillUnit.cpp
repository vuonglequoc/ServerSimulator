...
//	GS-N	2.00.05	JPN	0x00559910	-	Completed
//////////////////////////////////////////////////////////////////////
...
BOOL TMonsterSkillUnit::LoadData(const char* lpszFileName)
{
	TMonsterSkillUnit::s_bDataLoad = FALSE;

	if ( lpszFileName == NULL || strcmp(lpszFileName, "") == 0 )
	{
		CLog.MsgBox("[Monster Skill Unit] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Monster Skill Unit] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Monster SkillUnit] - Unit(%d) Error (%s) File. "
					...
					for(int i=0;i<5;i++)
					{
						...
						if ( iElement != -1 )
						{
							...
							if ( pMonsterSkillElement )
							{
								...
								CLog.LogAddC(TColor.Blue(), "[Monster Skill Unit] - Found SkillElement (%d)(%s)",
									iElement, pMonsterSkillElement->GetElementName());
							}
							else
							{
								CLog.LogAddC(TColor.Red(), "[Monster Skill Unit] - Not Found SkillElement (%d)",
									iElement);	
							}
						}
					}
		...
		fclose(SMDFile);
		CLog.LogAddC(TColor.Red(),"[Monster Skill Unit] - %s file is Loaded", lpszFileName);
		TMonsterSkillUnit::s_bDataLoad = TRUE;
	}
	catch(DWORD)
	{
		CLog.MsgBox("[Monster Skill Unit] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
TMonsterSkillUnit * TMonsterSkillUnit::FindSkillUnit(int iUnitNumber)
{
	if ( iUnitNumber < 0 || iUnitNumber >= MAX_MONSTER_SKILL_UNIT_ARRAY )
	{
		CLog.LogAdd("[Monster SkillUnit] FindSkillUnit() Error - (UnitNumber=%d) ",
			iUnitNumber);
...
void TMonsterSkillUnit::RunSkill(int iIndex, int iTargetIndex)
{
	...
	for ( int i=0;i<this->m_iElementsCount;i++)
	{
		...
		{
			CLog.LogAddC(TColor.Red(), "[Monster SkillUnit] SkillElement is NULL");
			continue;
		}
