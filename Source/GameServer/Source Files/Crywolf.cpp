...
//	GS-N	JPN	2.00.05	0x00568190	-	Completed
//////////////////////////////////////////////////////////////////////

#include "../Header Files/stdafx.h"
#include "../Header Files/Crywolf.h"
#include "../Header Files/CrywolfAltar.h"
#include "../Header Files/CrywolfStatue.h"
...
BOOL CCrywolf::LoadData(const char* lpszFileName)
{
	this->m_bFileDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, "") )
	{
		CLog.MsgBox("[Crywolf Main Job] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			CLog.MsgBox("[Crywolf Main Job] - Can't Open %s ", lpszFileName);
			return FALSE;
		}
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					{
						CLog.MsgBox("[Crywolf Main Job] - Excced MAX StartTime (%d)", this->m_StartTimeInfoCount);
					}
				...
				if ( iType == 2 )
				{
					...
					{
						CLog.MsgBox("[Crywolf Main Job] - Invalid Monster Group Number (%d)", iGroupNumber);
					}
				...
				if ( iType == 3 )
				{
					...
					if ( iGroupNumber <= -1 && iGroupNumber >= MAX_CRYWOLF_MONSTER_GROUP )	// #error Change && to ||
					{
						CLog.MsgBox("Error","[Crywolf Main Job] - Invalid Monster Boss Group Number (%d)", iGroupNumber);
					}
		...
		fclose(SMDFile);
		CLog.LogAddC(TColor.Red(), "[Crywolf Main Job] - %s file is Loaded", lpszFileName);
		this->m_bFileDataLoad = TRUE;
	}	// __try
	catch ( DWORD )
	{
		CLog.MsgBox("[Crywolf Main Job] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
void CCrywolf::LoadCrywolfMapAttr(const char* lpszFileName, int iOccupationState)
{
	if ( !lpszFileName )
		return;
...
void CCrywolf::SetCrywolfMapAttr(int iOccupationState)
{
	...
	{
		CLog.LogAddC(TColor.Red(), "[ Crywolf ] Fail - Chang Map Attribute (State:%d) ", iOccupationState);
		return;
	}
...
int CCrywolf::CheckStateTimeSync()
{
	...
	for ( int i=0;i<m_StartTimeInfoCount;i++)
	{
		...
		{
			CLog.LogAdd("[ Crywolf ] Start Crywolf");
...
{
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> NONE", this->m_iCrywolfState);
...
void CCrywolf::SetState_NOTIFY_1()
{
	...
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> NOTIFY_1", this->m_iCrywolfState);
...
void CCrywolf::SetState_NOTIFY_2()
{
	...
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> NOTIFY_2", this->m_iCrywolfState);
...
void CCrywolf::SetState_READY()
{
	...
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> READY", this->m_iCrywolfState);
...
void CCrywolf::SetState_START()
{
	...
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> START", this->m_iCrywolfState);
...
void CCrywolf::SetState_END()
{
	...
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> END", this->m_iCrywolfState);
	...
	else
	{
		...
		CLog.LogAdd("[ Crywolf ][MVP] SUCCESS.......!!");
	}
...
{
	CLog.LogAddC(TColor.Blue(), "[ Crywolf ] State (%d) -> ENDCYCLE", this->m_iCrywolfState);
...
void CCrywolf::ProcState_START()
{
		 ...
		 !this->m_bTurnUpBoss )
	{
		...
		CLog.LogAdd("[ Crywolf ][MVP] Barlgars TurnUp !!!");
	}
		 ...
		 !this->m_bChangeAI )
	{
		...
		CLog.LogAdd("[ Crywolf ][MVP] Start Monster Rush !!!");
	}
...
void CCrywolf::NotifyCrywolfCurrentState()
{
	...
	CLog.LogAddC(TColor.DarkGreen(), "[ Crywolf ] SetState - StateInfo : Occupation/%d, State/%d",
...
void CCrywolf::NotifyCrywolfStateLeftTime()
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x04, sizeof(pMsg));
	...
	CLog.LogAddC(TColor.DarkGreen(), "[ Crywolf ] War LeftTime - (%02d:%02d)",
...
void CCrywolf::NotifyCrywolfStatueAndAltarInfo()
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x02, sizeof(pMsg));
	...
	CLog.LogAdd("[ Crywolf ][Statue Info] Statue HP : %d",
	...
	for ( int iAltar = 205 ; iAltar <= 209 ; iAltar ++ )
	{
		...
		if ( iUserIndex != -1 )
		{
			CLog.LogAdd("[ Crywolf ][Altar Info] Altar(%d) Contractor: [%s][%s] ",
...
void CCrywolf::NotifyCrywolfBossMonsterInfo()
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x05, sizeof(pMsg));
	pMsg.iMonster1HP = -1;
	pMsg.btMonster2 = 0;
	...
	CLog.LogAdd("[ Crywolf ][Boss Monster Info] Balgars HP:%d, DarkElf:%d",
...
void CCrywolf::NotifyCrywolfStageEffectOnOff(BYTE btOnOff)
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x06, sizeof(pMsg));
...
void CCrywolf::NotifyCrywolfPersonalRank()
{
	...
	C1SubHeadSet((LPBYTE)&pMsg, 0xBD, 0x07, sizeof(pMsg));
	pMsg.btRank = 0;
	...
	for ( int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
			 ...
			 gObj[i].MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE)
		{
			...
			CLog.LogAdd("[ Crywolf ][Personal Rank & Exp.] [%s][%s] Score(%d) Rank(%d) Exp(%d)",
				gObj[i].AccountID, gObj[i].Name, gObj[i].m_iCrywolfMVPScore, pMsg.btRank, pMsg.iGettingExp);

			gSendProto.DataSend(i, (LPBYTE)&pMsg, sizeof(pMsg));
		}
	}
}
...
void CCrywolf::NotifyCrywolfHeroList()
{
	...
	for(int j=0;j<5&& _Itor != HeroSet.end();j++, _Itor++)
	{
		...
		CLog.LogAdd("[ Crywolf ][Hero List] [%s][%s] Score(%d) Ranking(%d)",
	...
	C1SubHeadSet((LPBYTE)lpMsg, 0xBD, 0x08, iHeroCount*sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO) + sizeof(PMSG_ANS_CRYWOLF_HERO_LIST_INFO_COUNT));
...
void CCrywolf::OperateGmCommand(int iUserIndex, int iCommand)
{
	...
	CLog.LogAdd("[ Crywolf ][Use GM Command] [%s][%s] Command : %d",
...
void CCrywolf::ApplyCrywolfDBInfo(int iCrywolfState, int iOccupationState)
{
	...
	CLog.LogAdd("[ Crywolf ][ ApplyDBInfo] OccupationState : %d",
		iOccupationState);
}
			...
			if ( lpMonObj->m_iCurrentAI != 0 )
			{
				if ( lpMonObj->Class == 349 )
				{
					if ( bExistKiller )
					{
						...
						CLog.LogAdd("[ Crywolf ][MVP] [Balgars Dead] by [%s][%s]",
					...
					else
					{
						...
						CLog.LogAdd("[ Crywolf ][MVP] [Balgars Dead] by Unknown User");
					}
				}
				...
				if ( lpMonObj->Class == 340 )
				{
					if ( bExistKiller )
					{
						...
						CLog.LogAdd("[ Crywolf ][MVP] [DarkElf Dead] by [%s][%s]",
							lpKillerObj->AccountID, lpKillerObj->Name);
					}
				}
...
void CCrywolf::MakeRewardForAltarElf(int iAltarUserIndex)
{
	...
	CLog.LogAdd("[ Crywolf ][Reward] [Altar Elf] [%s][%s] Success Reward - Gem Of Bless",
...
void CCrywolf::MakeRewardForHeroListTop5(int iUserIndex)
{
	...
	CLog.LogAdd("[ Crywolf ][Reward] [Hero List Top 5] [%s][%s] Success Reward - Gem Of Bless",
...
int CCrywolf::CalcGettingScore(int iUserIndex, int iMonIndex, int iScoreType)
{
	...
	CLog.LogAdd("[ Crywolf ][MVP] Plus MVP Score : %d, Accumulated : %d [%s][%s]",
...
void CCrywolf::GiveRewardExp(int iUserIndex, int iRewardExp)
{
	...
	if ( gObj[iUserIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			...
			if ( iMAX_LEVCOUNT > 5 )
			{
				CLog.LogAdd("[ Crywolf ][GiveRewardExp] LevelUp Error [%s][%s]",
					gObj[iUserIndex].AccountID, gObj[iUserIndex].Name);
