...
{
	g_DevilSquare.Enabled				= GetPrivateProfileInt(DEVILSQ_HDR,"Enabled",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS1ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS1ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS2ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS2ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS3ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS3ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS4ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS4ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS5ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS5ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS6ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS6ChaosSuccessRate",0,DEVIL_FILE_PATH);
	g_DevilSquare.DS7ChaosSuccessRate	= GetPrivateProfileInt(DEVILSQ_HDR,"DS7ChaosSuccessRate",0,DEVIL_FILE_PATH);
...
void CDevilSquare::Load(const char* filename)
{
	...
	if ( SMDFile == NULL )
	{
		CLog.MsgBox("[DevilSquare] Info file Load Fail [%s]", filename);
		return;
	}
	...
	CLog.LogAdd("%s file load!", filename);
}
...
void CDevilSquare::CheckSync()
{
	...
	CLog.LogAdd("[DevilSquare] Sync Open Time. [%d]min remain", this->m_iRemainTime);
}
				...
				for ( int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
				{
					if ( gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER)
					{
						if ( gUser.BloodCastleMapRange(gObj[i].MapNumber) == false )
						{
							if ( gUser.ChaosCastleMapRange(gObj[i].MapNumber) == false )
							{
								//gSendProto.DataSend(i, (BYTE *)&pNotice, pNotice.h.size);
							}
						}
					}
				}

				//CLog.LogAdd(pNotice.Notice);
	...
	else if ( this->m_iTime < GetTickCount() )
	{
		...
		if ( this->m_iRemainTime <= 15 )
		{
			if ( (this->m_iRemainTime%5)== 0 )
			{
				...
				//CLog.LogAdd((char*)pNotice.Notice);
			}
		}
...
void CDevilSquare::SetMonster()
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( DS_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE )
		{
			...
			if ( btDSIndex == 0xFF )
			{
				CLog.LogAdd("[DevilSquare] [%d] Invalid MonterType", wMonIndex);
				continue;
			}
...
void CDevilSquare::gDevilSquareMonsterRegen(LPOBJ lpObj)
{
	...
	{
		CLog.LogAdd("[DevilSquare] Invalid DevilSquareIndex [%d]", devilsquareindex);
		return;
	}
	...
	{
		CLog.LogAdd("[DevilSquare] [%d] Invalid MonterType", monstertype);
		return;
	}
	...
	CLog.LogAdd("[DevilSquare] Monter Regen [%d][%d][%d,%d]",
...
void CDevilSquare::SendEventStartMsg()
{
	...
	for ( int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].Connected == PLAYER_PLAYING )
		{
			if ( DS_MAP_RANGE(gObj[n].MapNumber) != FALSE )
			{
				//gSendProto.DataSend(n, (BYTE *)&pToEventer, pToEventer.h.size);
			}
			else
			{
				//gSendProto.DataSend(n, (BYTE *)&pWithOutEvneter, pWithOutEvneter.h.size);
			}
		}
	}

	CLog.LogAdd("[DevilSquare] Start Event");
}
...
void CDevilSquare::DieProcDevilSquare(LPOBJ lpObj)
{
	...
	if ( lpObj->Class == 1 )
	{
		if ( lpObj->Class >= 0 && lpObj->Class < MAX_CLASS_TYPE )
		{
	...
	CLog.LogAdd("[DevilSquare] Dead [%s][%s][%d][%d]",
...
int  CDevilSquare::gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg)
{
	...
	if ( lpObj->m_wExprienceRate == 0 )
		...
		exp = (int)((float)exp * ((float)lpObj->m_wExprienceRate  / 100.0f));
...
void CDevilSquare::gObjExpParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, BOOL MSBFlag)
{
	...
	bool bCheckSetParty[MAX_CLASS_TYPE];
	...
	// Gold Drop Rate
	int iGoldDropRate = GetPrivateProfileInt("MonsterDrops", "GoldDropMultiplier", 1, "..\\MMTData\\MMT_Items.ini");

	if(lpTargetObj->Type == OBJ_MONSTER)
	{
		//lpTargetObj->Money = (int)(totalexp * gAddExperience); // Fix Party Bug
		lpTargetObj->Money = (totalexp * iGoldDropRate); // Also fix party bug
	}
			...
			if ( lpTargetObj->MapNumber == lpPartyObj->MapNumber )
			{
				if ( dis[n] < 10 )
				{
					...
					if ( lpPartyObj->m_wExprienceRate == 0 )
						...
						exp = (int)((float)exp * ((float)lpPartyObj->m_wExprienceRate  / 100.0f));
	...
	for ( int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].Connected == PLAYER_PLAYING )
		{
			if ( DS_MAP_RANGE(gObj[n].MapNumber) != FALSE )
			{
				...
				{
					CLog.LogAddC(TColor.Red(), "[DevilSquare] [%s][%s] Found user in DevilSquare [State:Close]",
