...
#include "../Header Files/KanturuTowerOfRefinement.h"
...
BOOL CKanturuTowerOfRefinement::LoadData(const char * lpszFileName)
{
	this->m_bFileDataLoad = FALSE;

	if ( !lpszFileName || !strcmp(lpszFileName, ""))
	{
		CLog.MsgBox("[ KANTURU ][ TowerOfRefinement ] - File load error : File Name Error");
		return FALSE;
	}
	...
	try
	{
		...
		if ( SMDFile == NULL )
		{
			...
			CLog.MsgBox("[ KANTURU ][ TowerOfRefinement ] - Can't Open %s ",
				lpszFileName);
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 9 )
				{
					...
					{
						CLog.MsgBox("[ KANTURU ][ TowerOfRefinement ] - Exceed Max State Time (%d)",
		...
		CLog.LogAddC(2, "[ KANTURU ][ TowerOfRefinement ] %s file is Loaded",
			lpszFileName);
	...
	{
		CLog.MsgBox("[ KANTURU ][ TowerOfRefinement ] - Loading Exception Error (%s) File. ", lpszFileName);
	}
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> NONE",
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> REVITALIZATION",
		this->m_iTowerOfRefinementState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> NOTIFY1",
		this->m_iTowerOfRefinementState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> CLOSE",
		this->m_iTowerOfRefinementState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> NOTIFY2",
		this->m_iTowerOfRefinementState);
...
{
	CLog.LogAddC(5, "[ KANTURU ][ TowerOfRefinement ] State(%d) -> END",
		this->m_iTowerOfRefinementState);
	...
	for ( int iCount=0;iCount<OBJMAX;iCount++)
	{
			 ...
			 gObj[iCount].MapNumber == MAP_INDEX_KANTURU_BOSS )
		{
			...
			CLog.LogAdd("[ KANTURU ][ TowerOfRefinement ] State End:Kick Out [%s][%s]",
