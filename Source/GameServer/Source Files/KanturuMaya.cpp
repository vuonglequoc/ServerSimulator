...
#include "../Header Files/KanturuMaya.h"
...
void CKanturuMaya::KanturuMayaAct_IceStorm(int iSkillUsingRate)
{
	...
	CLog.LogAdd("[ KANTURU ][ IceStorm ] Skill Using(%d) : Index(%d) %s",
	...
	for ( int iCount=OBJ_STARTUSERINDEX;iCount < OBJMAX;iCount++)
	{
			 ...
			 gObj[iCount].MapNumber == MAP_INDEX_KANTURU_BOSS )
		{
			...
			if ( gObj[iCount].MapNumber == MAP_INDEX_KANTURU_BOSS )	// #warning unuseful if
			{
				...
				CLog.LogAdd("[ KANTURU ][ BrokenShower ] [%s][%s] User Dying cause NOT wearing MoonStone Pandent",
					gObj[iCount].AccountID, gObj[iCount].Name);
			}
		}
	}

	CLog.LogAdd("[ KANTURU ][ BrokenShower ] Skill Using : Index(%d) %s",
