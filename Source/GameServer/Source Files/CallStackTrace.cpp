...
#include "../Header Files/CallStackTrace.h"
#include "IMAGEHLP.H"

// GS-Normal 0x0048D6B1
//	GS-N	2.00.05	JPN	0x004A84F0
//	GS-CS	2.00.05	JPN	0x004A84F0
...
void CCallStackTrace::Output()
{
	...
	for ( int iC=0; iC < (int)this->m_strCallStackLog.size(); iC++)
...
int CCallStackTrace::TraceStack(BOOL bWriteInFile)
{
	...
	__try
	{
		...
		for ( UINT addr_index =0 ;addr_index < (UINT)stackframe.Far ; addr_index++ )
		{
			char symbol[0x504] ={0};
