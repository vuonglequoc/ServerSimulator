...
BOOL wsJoinServerCli::DataSend(PCHAR buf, int len)
{
	...
	while ( true )
	{
		...
		if ( nResult == -1)
		{
			...
			{
				CLog.LogAddC(2,"error-L3 : buffer error closed %d, %d", (*sendbuflen+nLeft), 819200);
				*sendbuflen = 0;
				this->Close(this->m_socket);
				return FALSE;
			}
...
BOOL wsJoinServerCli::FDWRITE_MsgDataSend()
{
	...
	while (*sendbuflen>0)
	{
		...
		if (nResult == -1)
		{
			if ( WSAGetLastError() != WSAEWOULDBLOCK)
			{
				CLog.LogAdd("FD_WRITE send() ¿¡·¯ %d %d", WSAGetLastError(), *sendbuflen);
				this->Close();
				return FALSE;
			}
			else
...
int wsJoinServerCli::DataRecv()
{
	...
	if ( nResult == 0 )
	{
		CLog.LogAdd("error-L3 : closed %d - buff:%d(%d) ",
	...
	*recvbuflen += nResult;
	...
	while ( true )
	{
		...
		{
			CLog.LogAdd("error-L2 : header error (%s %d)lOfs:%d, size:%d", __FILE__, __LINE__, lOfs, *recvbuflen);
			*recvbuflen = 0;
			return 0;
		}
